...
<span className='uppercase col-start-4 col-span-1 tracking-wider'> Quantity</span>
<span className='uppercase col-span-1 tracking-wider'> Total </span>
 ...
 {cartItems.length >= 1 && cartItems.map((item) => (
      ...
      <div className='h-48 my-5' >
       ...
       <div className='relative h-48'> <a className=''>
        <Image  className=' h-48 w-full object-cover group-hover:scale-105 
        transition-transfrom duration-500 ease-in-out
         cursor-pointer ' src={item.image} layout='fill' objectfit='cover' alt='well'  /> </a>
         </div>
       <div className='col-span-2 text-left flex flex-col'>
        <span className='uppercase text-2xl tracking-wider'>{item.name} </span>
        <span className='uppercase text-sm'>Black (S) </span> 
        <span onClick={() => removeItemHandler(item)} className='uppercase text-sm underline-offset-2 underline cursor-pointer hover:text-red-600'>Remove </span>  </div>
